...
class WeatherAPIWindField:
    ...
    def create_wind_field_from_api(self, 
                                   width: int, 
                                   height: int, 
                                   bbox: Tuple[float, float, float, float],
                                   resolution: float = 0.1) -> 'WindField':
        """
        ...
        wind_field = WindField(width=width, height=height)
    ...
    def _fetch_wind_grid(self, bbox: Tuple[float, float, float, float], resolution: float) -> Dict[str, Any]:
    ...
    def _populate_wind_field(self, wind_field: 'WindField', wind_data: Dict[str, Any], bbox: Tuple[float, float, float, float]):
        ...
        for i in range(height):
            for j in range(width):
                ...
                if hasattr(wind_field, 'wind_speed') and hasattr(wind_field, 'wind_direction'):
        ...
        print(f"Speed range: {wind_data['wind_speeds'].min():.1f} - {wind_data['wind_speeds'].max():.1f} knots")
        print(f"Average speed: {wind_data['wind_speeds'].mean():.1f} knots")


# Integration function for your existing code
def create_api_wind_field(map_size: list, 
                         bbox: Tuple[float, float, float, float],
                         api_key: Optional[str] = None) -> 'WindField':
    """
